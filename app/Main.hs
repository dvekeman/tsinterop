...
import Text.PrettyPrint (Doc, render, text, parens, nest, ($$), (<+>))
import Language.Kotlin (sepBy)
import Language.Kotlin (dot)

-- TODO: Union parameter types: create overloads

main :: IO ()
main = do
--  doMain "test-files/interface-simple.ts"
--  doMain "test-files/function-with-comment.d.ts"
--  doMain "test-files/namespace-with-function.d.ts"
--  doMain "test-files/namespace-with-function2.d.ts"
--  doMain "test-files/class.d.ts"
--  doMain "test-files/bigclass.d.ts"
--  doMain "test-files/namespace-with-type.d.ts"
--  doMain "test-files/partial.d.ts"
--  doMain "test-files/class-hierarchy.d.ts"
--  doMain "test-files/interface.d.ts"
--  doMain "test-files/function.d.ts"
--  doMain "test-files/typealias.d.ts"
--  doMain "test-files/innerclass.d.ts"
--  doMain "test-files/inheritance.d.ts"
--  doMain "test-files/inheritance2.d.ts"
--  doMain "test-files/union.d.ts"
--  doMain "test-files/function-optionals.d.ts"
  doMain "/src/SLIMS63/slimsclientkt/typings/smartclient.d.ts"

doMain :: String -> IO ()
doMain filename = do
  tsDecls <- parseFromFile TS.declarationSourceFile filename
--  let ctx = either (const initialContext) (\decls -> initialContext { declarations = decls }) tsDecls
  let ctx = either (const emptyAmbientContext) mkAmbientContext tsDecls
      ...
      eitherErrOrTypedef =
        KT.renderDeclarationSourceFile "isc" <$> ktDecls
--  print "--------------------------------------------------------------------------------"
...
--  print "--------------------------------------------------------------------------------"
  ...
  case ktDecls of
    Left err -> print err
    Right decls -> customWriteSourceFiles "isc" decls

-- Custom Rendering

customWriteSourceFiles :: String -> [KT.DeclarationElement] -> IO ()
customWriteSourceFiles qualifier els =
  mapM_ customWriteSourceFile $ customRenderDeclarationSourceFile qualifier els

customWriteSourceFile :: (String, String) -> IO ()
customWriteSourceFile (name, content) =
  writeFile ("out/" ++ name ++ ".isc.kt") content


customRenderDeclarationSourceFile :: String -> [KT.DeclarationElement] -> [(String, String)]
customRenderDeclarationSourceFile qualifier els =
  (\(name, doc) -> (name, render doc)) <$> customDeclarationSourceFile qualifier els
...
customDeclarationElement (KT.AmbientDeclaration _ _e a) =
  customRenderAmbientNamespace a
...
customRenderAmbientNamespace (KT.AmbientNamespaceDeclaration _ name ds) =
  let header =
        ( text "package" <+> sepBy dot text name
          $$ text ""
        )
...
tsNormalize _ctx ( TS.ExportDeclaration name ) =
  TS.ExportDeclaration name
...
tsInterfaceNormalize ctx (TS.Interface comment name mTypeParameters mTypeRefs typeBody) =
  TS.Interface (tsCommentNormalize comment) name (fmap (tsTypeParameterNormalize ctx) <$> mTypeParameters) (fmap (tsTypeRefNormalize ctx) <$> mTypeRefs) (tsTypeBodyNormalize ctx typeBody)
...
tsTypeMemberNormalize ctx (TS.ConstructSignature mTypeParameters parameters mType) =
  TS.ConstructSignature (fmap (tsTypeParameterNormalize ctx) <$> mTypeParameters) (tsParameterNormalize ctx <$> parameters) (tsTypeNormalize ctx <$> mType)
...
tsTypeNameNormalize (TS.TypeName mModuleName name) =
  TS.TypeName (tsModuleNameNormalize <$> mModuleName) name


tsModuleNameNormalize :: TS.ModuleName -> TS.ModuleName
tsModuleNameNormalize (TS.ModuleName names) =
  TS.ModuleName names
...
tsAmbientNormalize ctx _mParent (TS.AmbientFunctionDeclaration comment name parameterListAndReturnType) =
  TS.AmbientFunctionDeclaration (tsCommentNormalize comment) name (tsParameterListAndReturnTypeNormalize ctx parameterListAndReturnType)
...
tsAmbientNormalize ctx _mParent ambientModule@(TS.AmbientModuleDeclaration comment strings ambients) =
  TS.AmbientModuleDeclaration (tsCommentNormalize comment) strings (tsAmbientNormalize ctx (Just ambientModule) <$> ambients)
tsAmbientNormalize ctx _mParent ambientNamespace@(TS.AmbientNamespaceDeclaration comment strings ambients) =
  TS.AmbientNamespaceDeclaration (tsCommentNormalize comment) strings (tsAmbientNormalize ctx (Just ambientNamespace) <$> ambients)
tsAmbientNormalize ctx _mParent ambientExternalModule@(TS.AmbientExternalModuleDeclaration comment name ambientExternalModuleElements) =
  TS.AmbientExternalModuleDeclaration (tsCommentNormalize comment) name (tsAmbientExternalModuleElementNormalize ctx (Just ambientExternalModule) <$> ambientExternalModuleElements)
...
tsEntityNameNormalize (TS.EntityName mModuleName name) =
  TS.EntityName (tsModuleNameNormalize <$> mModuleName) name

tsConstEnumNormalize :: TS.ConstEnum -> TS.ConstEnum
tsConstEnumNormalize TS.ConstEnum = TS.ConstEnum

tsAmbientClassBodyElementNormalize :: AmbientContext -> Maybe TS.Ambient -> TS.AmbientClassBodyElement -> TS.AmbientClassBodyElement
...
tsAmbientClassBodyElementNormalize ctx _mParent (TS.AmbientMemberDeclaration mPublicOrPrivate mStatic name eitherMTypeOrParameterListAndReturnType) =
  TS.AmbientMemberDeclaration mPublicOrPrivate (tsStaticNormalize <$> mStatic) name (mapEither (fmap (tsTypeNormalize ctx)) (tsParameterListAndReturnTypeNormalize ctx) eitherMTypeOrParameterListAndReturnType)
...
tsAmbientExternalModuleElementNormalize :: AmbientContext -> Maybe TS.Ambient -> TS.AmbientExternalModuleElement -> TS.AmbientExternalModuleElement
...
tsAmbientExternalModuleElementNormalize _ctx _mParent (TS.ExportAssignment name) =
  TS.ExportAssignment name

tsParameterListAndReturnTypeNormalize :: AmbientContext -> TS.ParameterListAndReturnType -> TS.ParameterListAndReturnType
tsParameterListAndReturnTypeNormalize ctx (TS.ParameterListAndReturnType mTypeParameters parameters mType) =
  TS.ParameterListAndReturnType (fmap (tsTypeParameterNormalize ctx) <$> mTypeParameters) (tsParameterNormalize ctx <$> parameters) (tsTypeNormalize ctx <$> mType)
...
tsTypeParameterNormalize ctx (TS.TypeParameter name mType) =
  ...
  case maybeAnAliasType of
    ...
    Nothing ->
      TS.TypeParameter name (tsTypeNormalize ctx <$> mType)
...
tsParameterNormalize ctx (TS.RestParameter name mType) =
  TS.RestParameter name (tsTypeNormalize ctx <$> mType)
...
tsTypeNormalize ctx (TS.ObjectType typeBody) =
  TS.ObjectType (tsTypeBodyNormalize ctx typeBody)
tsTypeNormalize ctx (TS.ArrayType type_) =
  TS.ArrayType (tsTypeNormalize ctx type_)
tsTypeNormalize ctx (TS.UnionType type1@(TS.Predefined _ TS.AnyType) _type2) =
  tsTypeNormalize ctx type1
tsTypeNormalize ctx (TS.UnionType _type1 type2@(TS.Predefined _ TS.AnyType)) =
  tsTypeNormalize ctx type2
tsTypeNormalize ctx (TS.UnionType type1 type2) =
  TS.UnionType (tsTypeNormalize ctx type1) (tsTypeNormalize ctx type2)
tsTypeNormalize ctx (TS.TupleType types) =
  TS.TupleType (tsTypeNormalize ctx <$> types)
tsTypeNormalize _ctx (TS.TypeQuery strings) =
  TS.TypeQuery strings
tsTypeNormalize ctx (TS.FunctionType mTypeParameters parameters type_) =
  TS.FunctionType (fmap (tsTypeParameterNormalize ctx) <$> mTypeParameters) (tsParameterNormalize ctx <$> parameters) (tsTypeNormalize ctx type_)
tsTypeNormalize ctx (TS.ConstructorType mTypeParameters parameters type_) =
  TS.ConstructorType (fmap (tsTypeParameterNormalize ctx) <$> mTypeParameters) (tsParameterNormalize ctx <$> parameters) (tsTypeNormalize ctx type_)

tsParameterTypeNormalize :: AmbientContext -> TS.ParameterType -> TS.ParameterType
tsParameterTypeNormalize ctx (TS.ParameterType type_) =
  TS.ParameterType (tsTypeNormalize ctx type_)
tsParameterTypeNormalize _ctx (TS.ParameterSpecialized string) =
  TS.ParameterSpecialized string
...
ts2kt _ctx ( TS.ExportDeclaration name ) =
  KT.ExportDeclaration name
...
tsTypeAlias2Kt ctx (TS.TypeAlias comment name type_) =
  KT.TypeAlias (tsComment2Kt comment) name (tsType2Kt ctx type_)
...
tsInterface2Kt ctx (TS.Interface comment name mTypeParameters mTypeRefs typeBody) =
  KT.Interface (tsComment2Kt comment) name (fmap (tsTypeParameter2Kt ctx) <$> mTypeParameters) (fmap (tsTypeRef2Kt ctx) <$> mTypeRefs) (tsTypeBody2Kt ctx typeBody)

tsTypeRef2Kt :: AmbientContext -> TS.TypeRef -> KT.TypeRef
tsTypeRef2Kt ctx (TS.TypeRef typeName mTypes) =
  KT.TypeRef (tsTypeName2Kt typeName) (fmap (tsType2Kt ctx) <$> mTypes)
...
tsTypeMember2Kt ctx (TS.ConstructSignature mTypeParameters parameters mType) =
  KT.ConstructSignature (fmap (tsTypeParameter2Kt ctx) <$> mTypeParameters) (tsParameter2Kt ctx <$> parameters) (tsType2Kt ctx <$> mType)
...
tsTypeName2Kt (TS.TypeName mModuleName name) =
  KT.TypeName (tsModuleName2Kt <$> mModuleName) name


tsModuleName2Kt :: TS.ModuleName -> KT.ModuleName
tsModuleName2Kt (TS.ModuleName names) =
  KT.ModuleName names
...
tsAmbient2Kt ctx _mParent (TS.AmbientFunctionDeclaration comment name parameterListAndReturnType) =
  KT.AmbientFunctionDeclaration (tsComment2Kt comment) name (tsParameterListAndReturnType2Kt ctx parameterListAndReturnType)
...
tsAmbient2Kt ctx _mParent ambientExternalModule@(TS.AmbientExternalModuleDeclaration comment name ambientExternalModuleElements) =
  KT.AmbientExternalModuleDeclaration (tsComment2Kt comment) name (tsAmbientExternalModuleElement2Kt ctx (Just ambientExternalModule) <$> ambientExternalModuleElements)
...
tsAmbientClassBodyElement2Kt :: AmbientContext -> Maybe TS.Ambient -> TS.AmbientClassBodyElement -> KT.AmbientClassBodyElement
...
tsAmbientClassBodyElement2Kt ctx mParent member@(TS.AmbientMemberDeclaration mPublicOrPrivate mStatic name eitherMTypeOrParameterListAndReturnType) =
  let (_siblingMembers, inheritedClassMembers, inheritedInterfaceMembers) = maybe ([], [], []) (ambientClassMembers ctx) mParent
      isOverride =
        overridesFromClass member inheritedClassMembers
        || overridesFromInterface member inheritedInterfaceMembers
--      isOverload =
...
--  KT.AmbientMemberDeclaration isOverride (tsPublicOrPrivate <$> mPublicOrPrivate) (tsStatic2Kt <$> mStatic) name overloadedName (mapEither (fmap tsType2Kt) tsParameterListAndReturnType2Kt eitherMTypeOrParameterListAndReturnType)
  KT.AmbientMemberDeclaration isOverride (tsPublicOrPrivate <$> mPublicOrPrivate) (tsStatic2Kt <$> mStatic) name (mapEither (fmap (tsType2Kt ctx)) (tsParameterListAndReturnType2Kt ctx) eitherMTypeOrParameterListAndReturnType)
...
tsAmbientExternalModuleElement2Kt :: AmbientContext -> Maybe TS.Ambient -> TS.AmbientExternalModuleElement -> KT.AmbientExternalModuleElement
...
tsAmbientExternalModuleElement2Kt _ctx _mParent (TS.ExportAssignment name) =
  KT.ExportAssignment name
...
tsParameterListAndReturnType2Kt ctx (TS.ParameterListAndReturnType mTypeParameters parameters mType) =
  KT.ParameterListAndReturnType (fmap (tsTypeParameter2Kt ctx) <$> mTypeParameters) (tsParameter2Kt ctx <$> parameters) (tsType2Kt ctx <$> mType)
...
tsTypeParameter2Kt ctx (TS.TypeParameter name mType) =
  KT.TypeParameter name (tsType2Kt ctx <$> mType)
...
tsParameter2Kt ctx (TS.RequiredOrOptionalParameter mPublicOrPrivate name mOptional mParameterType) =
  KT.RequiredOrOptionalParameter (tsPublicOrPrivate <$> mPublicOrPrivate) name (tsOptional2Kt <$> mOptional) (tsParameterType2Kt ctx <$> mParameterType)
tsParameter2Kt ctx (TS.RestParameter name mType) =
  KT.RestParameter name (tsType2Kt ctx <$> mType)
...
tsType2Kt _ctx (TS.TypeReference (TS.TypeRef (TS.TypeName _ "Partial") _)) =
  KT.Predefined (Left (0, 0)) KT.AnyType
...
tsType2Kt ctx (TS.ObjectType typeBody) =
  KT.ObjectType (tsTypeBody2Kt ctx typeBody)
...
tsType2Kt ctx unionType@(TS.UnionType _ _) =
  ...
  case unionTypes of
    ...
    _ ->
      ...
      in
      if all (compareTypes firstType) unionTypes
      then tsType2Kt ctx $ joinUnionTypes unionTypes
      else KT.DynamicAliasType (Just (tsType2Kt ctx <$> unionTypes))
tsType2Kt ctx (TS.TupleType types) =
  KT.TupleType (tsType2Kt ctx <$> types)
tsType2Kt _ctx (TS.TypeQuery strings) =
  KT.TypeQuery strings
tsType2Kt ctx (TS.FunctionType mTypeParameters parameters type_) =
  KT.FunctionType (fmap (tsTypeParameter2Kt ctx) <$> mTypeParameters) (tsParameter2Kt ctx <$> parameters) (tsType2Kt ctx type_)
tsType2Kt ctx (TS.ConstructorType mTypeParameters parameters type_) =
  KT.ConstructorType (fmap (tsTypeParameter2Kt ctx) <$> mTypeParameters) (tsParameter2Kt ctx <$> parameters) (tsType2Kt ctx type_)

tsParameterType2Kt :: AmbientContext -> TS.ParameterType -> KT.ParameterType
tsParameterType2Kt ctx (TS.ParameterType type_) =
  KT.ParameterType (tsType2Kt ctx type_)
tsParameterType2Kt _ctx (TS.ParameterSpecialized string) =
  KT.ParameterSpecialized string
...
joinUnionTypes (t1:t2:rest) =
  joinUnionTypes (joinUnionTypes_ t1 t2:rest)
...
joinUnionTypes_ (TS.Predefined comment1 (TS.StringType mTypes1)) (TS.Predefined comment2 (TS.StringType mTypes2)) =
  TS.Predefined (joinComments comment1 comment2) (TS.StringType $
...
joinUnionTypes_ (TS.Predefined comment1 type1) (TS.Predefined comment2 type2) =
  ...
  else error $ "Types must be the same. '" ++ show type1 ++ "' != '" ++ show type2 ++ "'" ++ "\n" ++ show comment1 ++ "\n" ++ show comment2
joinUnionTypes_ t1@(TS.Predefined _ _) (TS.UnionType t2 u) =
  joinUnionTypes_ t1 (joinUnionTypes_ t2 u)
  ...
  let ownMembers = catMaybes $ ambientInterfaceBodyMember . snd <$> els
      ...
      inheritedMembers = -- Starting from the 'extends' clause, find all functions in the parent declarations
        ...
        concat $
            ...
            joinTupleLists . ambientInterfaceMembers ctx <$>
              ambientClassParents ctx allTypeRefs
   in
--      trace (
--        "INTERFACE " ++ _name
--        ++ "\n"
...
ambientClassMembers :: AmbientContext -> TS.Ambient -> ([TS.AmbientClassBodyElement], [TS.AmbientClassBodyElement], [TS.TypeMember])
ambientClassMembers ctx (TS.AmbientClassDeclaration _ _name _ mExtendsTypeRefs mImplementsTypeRef els) =
  let ownMembers = ambientClassBodyMember . snd =<< els
      ...
      inheritedAmbientClassMembers =
        foldr (\(classInherits, interfaceInherits) (classInheritsAcc, interfaceInheritsAcc) -> (classInherits ++ classInheritsAcc, interfaceInherits ++ interfaceInheritsAcc)) ([], [])
          -- We don't care where the inherited function comes from (parent vs parent of parent vs ...)
...
--      )
      (ownMembers, fst inheritedAmbientClassMembers, snd inheritedAmbientClassMembers)
...
ambientClassBodyMember member@TS.AmbientMemberDeclaration {} =
  [member]
ambientClassBodyMember member@TS.AmbientMemberDeclaration {} =
--ambientClassBodyMember member@(TS.AmbientMemberDeclaration mPubPriv mStatic name (Right (TS.ParameterListAndReturnType mTypeParams params mType))) =
  -- Create permutations of optional vs non-optional params
  -- and generate a member for all of these
...
--    optionalPermutations isOptionalParameter params
  [member]
ambientClassBodyMember member@TS.AmbientIndexSignature {} =
  [member]
ambientClassBodyMember _ =
  []

isOptionalParameter :: TS.Parameter -> Bool
isOptionalParameter (TS.RequiredOrOptionalParameter _mPubPriv _name (Just TS.Optional) _mParamType) =
  True
...
ambientClassParents ctx typeRefs =
  concat $ findAmbientByName ctx <$> typeRefs

findAmbientByName :: AmbientContext -> TS.TypeRef -> [ TS.Ambient ]
findAmbientByName ctx (TS.TypeRef tname _) =
  findAmbientByName_ tname (declarations ctx ++ inheritedDeclarations ctx)

findAmbientByName_ :: TS.TypeName -> [TS.DeclarationElement] -> [ TS.Ambient ]
findAmbientByName_ typeName els =
  concat $ ambientByNameMatch typeName <$> els

ambientByNameMatch :: TS.TypeName -> TS.DeclarationElement -> [TS.Ambient]
ambientByNameMatch typeName (TS.AmbientDeclaration _ _ ambient ) =
  ambientByNameMatchAmbient_ typeName ambient
...
ambientByNameMatchAmbient typeName ambients =
  concat $ ambientByNameMatchAmbient_ typeName <$> ambients
...
debugTypeName (TS.TypeName Nothing name) =
  "TypeName::" ++ name
...
--  )
  ...
  && compareMaybes compareParameterListAndReturnType (ambientClassBodyElementParams type1) (ambientClassBodyElementParams inheritedType)

overridesFromInterface :: TS.AmbientClassBodyElement -> [TS.TypeMember] -> Bool
overridesFromInterface function@TS.AmbientMemberDeclaration {} inherited = any (overridesFromInterface_ function) inherited
...
ambientClassBodyElementName (TS.AmbientMemberDeclaration _mPublicOrPrivate _mStatic name _eitherMTypeOrParameterListAndReturnType) = name
...
ambientClassBodyElementParams :: TS.AmbientClassBodyElement -> Maybe (Either (Maybe TS.Type) TS.ParameterListAndReturnType)
ambientClassBodyElementParams (TS.AmbientConstructorDeclaration _) = Nothing
ambientClassBodyElementParams (TS.AmbientMemberDeclaration _mPublicOrPrivate _mStatic _name eitherMTypeOrParameterListAndReturnType) = Just eitherMTypeOrParameterListAndReturnType
...
compareTypeNames (TS.TypeName Nothing name1) (TS.TypeName Nothing name2) =
  name1 == name2
compareTypeNames (TS.TypeName (Just moduleName1) name1) (TS.TypeName (Just moduleName2) name2) =
  compareModuleNames moduleName1 moduleName2
  && name1 == name2
compareTypeNames _ _ =
  False

compareModuleNames :: TS.ModuleName -> TS.ModuleName -> Bool
compareModuleNames (TS.ModuleName names1) (TS.ModuleName names2) =
  names1 == names2

compareParameterListAndReturnType :: Either (Maybe TS.Type) TS.ParameterListAndReturnType -> Either (Maybe TS.Type) TS.ParameterListAndReturnType -> Bool
...
--  )
  result
compareParameterListAndReturnType (Left mType1) (Left mType2) =
  compareMaybes compareTypes mType1 mType2
...
compareTypes _ (TS.Predefined _ TS.AnyType) =
  True
...
compareTypes (TS.TypeReference typeRef1) (TS.TypeReference typeRef2) =
  compareTypeRefs typeRef1 typeRef2
compareTypes (TS.ObjectType typeBody1) (TS.ObjectType typeBody2) =
  compareTypeBodies typeBody1 typeBody2
compareTypes (TS.ArrayType type1) (TS.ArrayType type2) =
...
compareTypes (TS.TupleType types1) (TS.TupleType types2) =
  compareLists compareTupleTypes types1 types2
compareTypes (TS.TypeQuery queries1) (TS.TypeQuery queries2) =
  queries1 == queries2
...
compareParameterTypes (TS.ParameterType type1) (TS.ParameterType type2) =
  compareTypes type1 type2
compareParameterTypes (TS.ParameterSpecialized typeName1) (TS.ParameterSpecialized typeName2) =
  typeName1 == typeName2
...
compareTypeRefs (TS.TypeRef typeName1 (Just types1)) (TS.TypeRef typeName2 (Just types2)) =
  compareTypeNames typeName1 typeName2
  && compareLists compareTypeTuples types1 types2
compareTypeRefs (TS.TypeRef typeName1 Nothing) (TS.TypeRef typeName2 Nothing) =
  compareTypeNames typeName1 typeName2
...
compareTypeMembers ((_, typeMember1), (_, typeMember2)) =
  compareTypeMembers_ typeMember1 typeMember2
...
compareIndexSignatures (TS.IndexSignature _name1 stringOrNumber1 type1) (TS.IndexSignature _name2 stringOrNumber2 type2) =
  compareStringOrNumbers stringOrNumber1 stringOrNumber2
  && compareTypes type1 type2
...
mkAlias' (TS.AmbientMemberDeclaration _pubPriv {--static:--}Nothing name (Right (TS.ParameterListAndReturnType mTypeParams params (Just type_))))
  | not (null name) && isUpper (head name) =
--      trace ("mkAlias' for member '" ++ name ++ "'")
      [(name, TS.FunctionType mTypeParams params type_)]
  | otherwise = []
mkAlias' _ = []
...
mapAlias_ ambientContext mPreviousAlias aliasedName =
  let aliases = typeAliases ambientContext
      maybeANewAlias =
        listToMaybe $
          map snd $
           filter (\(name, _t) -> name == aliasedName) aliases
  in
--  trace ("mapAlias for " ++ aliasedName) $
  case maybeANewAlias of
